...
public class OreSim extends GameGrid implements GGKeyListener
{
  ...
  public interface Movable {
    ...
    /**
     * interface to if the vehicle is movable to the location
     */
  ...
  public abstract class Vehicle extends Actor implements Movable {
    ...
    protected int autoMovementIndex = 0;
    protected int moveCount = 0;
    protected int id; // Unique ID for each vehicle - for future development

    public Vehicle(String dir) {
      super(true, dir);  // Assuming all vehicles are rotatable and have a sprite
    }
    protected abstract void setupVehicle(boolean isAutoMode, List<String> controls);
  ...
  private class Pusher extends Vehicle
  {
    public Pusher()
    {
      super("sprites/pusher.png");  // Rotatable
    }


    @Override
    protected void setupVehicle(boolean isAutoMode, List<String> controls) {
  ...
  private class Bulldozer extends Vehicle
  {
    private int clayRemoved;
    public Bulldozer()
    {
      super( "sprites/bulldozer.png");  // Rotatable
      clayRemoved = 0;
    }
    @Override
    protected void setupVehicle(boolean isAutoMode, List<String> controls) {
    ...
    public boolean canMove(Location location) {
      ...
      Rock rock = (Rock)getOneActorAt(location, Rock.class);
      Ore ore = (Ore)getOneActorAt(location, Ore.class);
      Pusher pusher = (Pusher)getOneActorAt(location, Pusher.class);
      Excavator excavator = (Excavator)getOneActorAt(location, Excavator.class);
      if (c.equals(borderColor) || rock != null || ore != null || pusher != null || excavator != null) {
        return false;
      } else {
        Clay clay = (Clay)getOneActorAt(location, Clay.class);
        if (clay != null) {
  ...
  private class Excavator extends Vehicle
  {
    private int rocksRemoved;

    public Excavator()
    {
      super("sprites/excavator.png");  // Rotatable
      rocksRemoved = 0;
    }
    @Override
    protected void setupVehicle(boolean isAutoMode, List<String> controls) {
    ...
    public boolean canMove(Location location) {
      ...
      Ore ore = (Ore)getOneActorAt(location, Ore.class);
      Pusher pusher = (Pusher)getOneActorAt(location, Pusher.class);
      Excavator otherExcavator = (Excavator)getOneActorAt(location, Excavator.class);
      if (c.equals(borderColor) || clay != null || ore != null || pusher != null || otherExcavator != null) {
        return false;
      } else {
        Rock rock = (Rock)getOneActorAt(location, Rock.class);
        if (rock != null) {
  ...
  private class Obstacle extends Actor {
  ...
  private class Clay extends Obstacle
  {
  ...
  /**
   * For future development where each type has multiple machines
   */
  ...
  private int pusherIndex = 0;
  private int bulldozerIndex = 0;
    ...
    while(oresDone < grid.getNbOres() && gameDuration >= 0) {
      try {
        ...
        if (isAutoMode) {
          ...
          // check if bulldozer and excavator are present
          if (bulldozer != null) {
  ...
  private void updateStatistics() {
    ...
    try {
      ...
      fileWriter.write("Pusher-" + pusherIndex + " Moves: " + pusher.getMoveCount() + "\n");
      if (excavator != null) {
        fileWriter.write("Excavator-" + excavatorIndex + " Moves: " + excavator.getMoveCount() + "\n");
        fileWriter.write("Excavator-" + excavatorIndex + " Rock removed: " + excavator.getRocksRemoved() + "\n");
      }
      if (bulldozer != null) {
        fileWriter.write("Bulldozer-" + bulldozerIndex + " Moves: " + bulldozer.getMoveCount() + "\n");
        fileWriter.write("Bulldozer-" + bulldozerIndex + " Clay removed: " + bulldozer.getClayRemoved() + "\n");
      }
      /**
       * for future development where each type of vehicle has multiple machines
       */
      /*
      for (Pusher pusher : pushers) {
        fileWriter.write("Pusher-" + pusher.getId() + " Moves: " + pusher.getMoveCount() + "\n");
      }
      for (Bulldozer bulldozer : bulldozers) {
        fileWriter.write("Bulldozer-" + bulldozer.getId() + " Moves: " + bulldozer.getMoveCount() + "\n");
        fileWriter.write("Bulldozer-" + bulldozer.getId() + " Clay removed: " + bulldozer.getClayRemoved() + "\n");
      }
      for (Excavator excavator : excavators) {
        fileWriter.write("Excavator-" + excavator.getId() + " Moves: " + excavator.getMoveCount() + "\n");
        fileWriter.write("Excavator-" + excavator.getId() + " Rocks removed: " + excavator.getRocksRemoved() + "\n");
      }
      */
    } catch (IOException e) {
    ...
    for (int y = 0; y < nbVertCells; y++)
    {
      for (int x = 0; x < nbHorzCells; x++)
      {
        ...
        if (a == ElementType.PUSHER)
        {
          ...
          pusher.setupVehicle(isAutoMode, controls);
        }
        ...
        if (a == ElementType.BULLDOZER)
        {
          ...
          bulldozer.setupVehicle(isAutoMode, controls);
        }
        if (a == ElementType.EXCAVATOR)
        {
          ...
          excavator.setupVehicle(isAutoMode, controls);
        }
      }
    }
